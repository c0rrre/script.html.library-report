...
DEFAULT_DATA = xbmc.translatePath(ADDON.getAddonInfo('profile'))
...
def main():
    ...
    f_http.write(
        'h1 {color:DeepSkyBlue; text-align:center; margin-top: 0.42em; font-family:Arial Black; text-transform:uppercase}\n')
    f_http.write('p.date {color:white; margin:0; padding-top:12px; text-align:right; font-family:Arial}\n')
    f_http.write(
        'table.gridtable {font-family:Arial; font-size:14.5px; border-width:1px; border-color:Black; border-collapse:collapse; width:98%; margin:auto}\n')
    f_http.write(
        'table.gridtable th {font-size:20px; font-family:Arial Black; text-transform: uppercase; border-width:1px; padding:8px; border-style:solid; border-color:Black; background-color:Silver}\n')
    f_http.write(
        'table.gridtable td {border-width:1px; padding:8px; border-style:solid; border-color:Black; background-color:White}\n')
    f_http.write('a.anchor{display: block; position:relative; top:-60px; visibility: hidden}\n')
    f_http.write(
        'a:link {color:Black} a:visited {color:Black} a:hover {color:Black} a:active {color:Black} a:hover {background:White} a:focus {outline:none}\n')
    f_http.write(
        '.github a:link {color:DeepSkyBlue} .github a:visited {color:DeepSkyBlue} .github a:hover {color:YellowGreen} .github a:active {color:YellowGreen} .github a:hover {background:none}\n')
    f_http.write(
        '.button {display:inline-block; font-weight:bold; font-size:15px; width:110px; height:20px; background:Silver; padding-top:3.5px; text-align:center; border-radius:4px; text-transform:uppercase; text-decoration:none; font-family:Arial}\n')
    ...
    if (include_movies == 'true') and (include_tvshows == 'true'):
        f_http.write('<a href="#movie_link" class="button">' + xbmc.getLocalizedString(
            342) + '</a>&nbsp;&nbsp;<a href="#tvshow_link" class="button">' + xbmc.getLocalizedString(20343) + '</a>\n')
    f_http.write('</div>\n')
    ...
    if (include_movies == 'true') and xbmc.getCondVisibility("Library.HasContent(Movies)"):
        ...
        for movie in movies:
            ...
            if movie['playcount'] > 0:
                ...
                watched = ''
            f_http.write('<tr>\n')
            f_http.write('<td width="27.5%"><b><a href="http://www.imdb.com/title/' + str(
                movie['imdbnumber']) + '/" target="_blank">' + movie['label'] + '</a></b> (' + str(
                movie['year']) + ')</td>\n')
    ...
    if (include_tvshows == 'true') and xbmc.getCondVisibility("Library.HasContent(TVShows)"):
        ...
        f_http.write('<table class="gridtable">\n')
        f_http.write('<tr>\n')
        f_http.write('<th colspan="9">' + xbmc.getLocalizedString(20343) + ' (' + '{:,}'.format(
            len(tvshows)) + ') - ' + xbmc.getLocalizedString(20360) + ' (' + '{:,}'.format(len(episodes)) + ')</th>\n')
        f_http.write('</tr>\n')
        pDialog.create(LANGUAGE(30033), '')
        tvcount = 0
        for tvshow in tvshows:
            ...
            if tvshow['playcount'] > 0:
                ...
                watched = ''
            f_http.write('<tr>\n')
            f_http.write('<td width="27.5%"><b><a href="http://thetvdb.com/?tab=series&amp;id=' + str(
                tvshow['imdbnumber']) + '/" target="_blank">' + tvshow['label'] + '</a></b> (' + str(
                tvshow['year']) + ')</td>\n')
            if HD_Show:
            ...
            f_http.write('<td width="13%">' + xbmc.getLocalizedString(33054) + ' ' + str(
                tvshow['season']) + ' / ' + xbmc.getLocalizedString(20360) + ' ' + str(tvshow['episode']) + '</td>\n')
            if tvshow['mpaa'] == "":
                f_http.write('<td width="8%">' + LANGUAGE(30014) + ' NA</td>\n')
            else:
    ...
    f_http.write(
        '<p class="github" style="color:white; font-family:Arial; margin:0">GitHub: <a href="https://github.com/Steveb1968/script.html.library-report" target="_blank">' + ADDON_ID + '</a></p>\n')
    f_http.write('</div>\n')
    f_http.write('</div>\n')
    f_http.write('</body>\n')
    f_http.write('</html>')
